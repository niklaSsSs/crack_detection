"""
SDNET2018 Encoder Pre-training for Crack Detection
Binary classification (Crack vs NonCrack) using EfficientNet-B0 backbone
"""
...
CONFIG = {
    ...
    'experiment_name': f'efficientnet_b0_crack_pretraining_{datetime.now().strftime("%Y%m%d_%H%M%S")}',

    # Model
    'backbone': 'efficientnet_b0',
    'pretrained': True,
    'input_size': 512,  # Resize from 256 to 512
...
def get_train_transforms(input_size=512):
    ...
    return A.Compose([
        ...
        A.Rotate(limit=7, border_mode=0, p=0.3),
        A.ShiftScaleRotate(
            shift_limit=0.05,
            scale_limit=0.1,
            rotate_limit=7,
...
def extract_series_id(file_path):
    """Extract series ID from filename for grouped splitting"""
    # SDNET2018 format: 7001-115.jpg -> series "7001"
...
def load_dataset():
    """Load dataset with domain and series information"""
...
def create_grouped_split(file_paths, labels, domains, series_ids):
    """Create train/val/test split grouped by series"""
    ...
    n_splits = 10  # Use 10-fold, take first fold as test
    skf = StratifiedGroupKFold(n_splits=n_splits, shuffle=True, random_state=SEED)

    # Get train and temp (val+test) indices
    ...
    for split_name, split_data in splits.items():
        ...
        for domain in ['Bridge', 'Pavement', 'Wall']:
            domain_mask = split_data['domains'] == domain
            if domain_mask.sum() > 0:
                domain_crack = ((split_data['labels'] == 1) & domain_mask).sum()
                domain_total = domain_mask.sum()
                print(f"  {domain}: {domain_total} ({domain_crack} crack, {domain_total-domain_crack} noncrack)")
...
class CrackClassifier(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        # Replace classifier with identity (we'll use features)
    ...
    def forward(self, x):
        ...
        # Reshape if needed (EfficientNet outputs [B, C, 1, 1])
    ...
    def get_encoder(self):
        """Return encoder weights for transfer learning"""
        return self.encoder
...
def main():
    ...
    print("=" * 80)
    print("SDNET2018 ENCODER PRE-TRAINING")
    print("=" * 80)
    ...
    print(f"\nLoss: BCEWithLogitsLoss with pos_weight={pos_weight.item():.2f}")
    ...
    for epoch in range(1, CONFIG['epochs'] + 1):
        ...
        # Domain-specific metrics
        print("\nVal Domain Metrics:")
        for domain, metrics in val_metrics['domain_metrics'].items():
            auc_str = f"{metrics['auc']:.4f}" if metrics['auc'] is not None else "N/A"
            print(f"  {domain:10} - AUC: {auc_str}, F1: {metrics['f1']:.4f}, N: {metrics['n_samples']}")
    ...
    # Usage instructions
    usage_note = f"""
# Encoder Weights Usage Instructions

## Model Info
- Backbone: {CONFIG['backbone']}
- Pre-trained on: SDNET2018 (56K images, crack detection)
...
## How to Use for Weld Seam Segmentation
...
# Initialize your segmentation model (U-Net/DeepLabv3+)
# Replace the encoder with pre-trained weights
# Example for U-Net:
...
## Important Notes
- Images must be resized to 512x512
- Use the same normalization (ImageNet stats)
- The encoder expects RGB images (3 channels)
- Consider domain adaptation if weld images differ significantly from concrete cracks
"""

    with open(output_dir / 'USAGE.md', 'w') as f:
        f.write(usage_note)

    print(f"✓ Usage instructions saved: {output_dir / 'USAGE.md'}")
    ...
    print(f"  Val AUC ≥ 0.95: {'✓' if best_val_auc >= 0.95 else '✗'} ({best_val_auc:.4f})")
    print(f"  Test AUC ≥ 0.95: {'✓' if test_metrics['auc'] >= 0.95 else '✗'} ({test_metrics['auc']:.4f})")

    # Check domain robustness
    min_domain_auc = min(
        m['auc'] for m in test_metrics['domain_metrics'].values() if m['auc'] is not None
    )
    print(f"  Min Domain AUC: {min_domain_auc:.4f}")
